...
const L = 9, M = 999999999, 
ops = { '+': (a, b) => a + b, '-': (a, b) => a - b, '×': (a, b) => a * b, '÷': (a, b) => a / b, '%': (a, b) => a % b }
export const useCalculator = () => {
  ...
  const press = k => {
    if ('0123456789.'.includes(k)) {setD(p => 
      (p === '0' || (o && a === null) || s || p === 'ERROR' || (p.includes('.') && k === '.') ? '' : p).length < L ? (
          (p === '0' || p == 'ERROR') || (p.includes('.') && k === '.') || s ? '' : p
      ) + k : p); setS(false) }
